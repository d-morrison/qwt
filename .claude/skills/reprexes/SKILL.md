...
- The **`reprex` package** (tidyverse, <https://reprex.tidyverse.org/>)
  ...
  non-interactive CLI session where there's no clipboard.
  Use `reprex()` when the output is destined for a PR comment or an upstream
  issue. Useful arguments:
  - `venue =` — output format:
  ...
  Companion helpers handle "wild-caught" reprexes: `reprex_clean()` (strip
  the prompts/output from a copied reprex), `reprex_rescue()` (recover code
  from R-console output with `>`/`+` prompts), and `reprex_invert()` (turn a
  rendered reprex back into plain code).
